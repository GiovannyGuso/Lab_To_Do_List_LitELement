...
class TaskItem extends LitElement {
  ...
  render() {
    ...
    return html`
      <div class="task-item ${this.task.completed ? 'completed' : ''}" 
           @click=${e => !e.target.closest('.task-actions') && this._dispatchEvent('task-selected', { task: this.task })}>
        ...
        <div class="task-actions">
          <button class="action-btn edit" @click=${e => { e.stopPropagation(); this._dispatchEvent('task-edit', { task: this.task }); }} 
                  title="Editar tarea">
            <svg width="20" height="20" viewBox="0 0 256 256" fill="currentColor">
              <path d="M227.31,73.37,182.63,28.68a16,16,0,0,0-22.63,0L36.69,152A15.86,15.86,0,0,0,32,163.31V208a16,16,0,0,0,16,16H92.69A15.86,15.86,0,0,0,104,219.31L227.31,96a16,16,0,0,0,0-22.63ZM92.69,208H48V163.31l88-88L180.69,120ZM192,108.68,147.31,64l24-24L216,84.68Z"/>
            </svg>
          </button>
          <button class="action-btn delete" @click=${e => { e.stopPropagation(); this._handleDelete(); }} 
  ...
  _handleDelete() {
    if (confirm(`¿Estás seguro de que deseas eliminar la tarea "${this.task.name}"?`)) {
      this._dispatchEvent('task-deleted', { task: this.task });
    }
  }
}
